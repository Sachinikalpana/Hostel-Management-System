...
if(isset($_POST['update_student'])){
    ...
    if(!preg_match('/^\d{9}$/', $phone_digits)){
        ...
        try {
            // Get Student Room
            $get_current = $conn->prepare("SELECT room_number FROM users WHERE id = ? AND role = 'student'");
            $get_current->bind_param('i', $student_id);
            $get_current->execute();
            $current_result = $get_current->get_result();
            $current_room = null;
            if($current_result->num_rows > 0) {
                $current_room = $current_result->fetch_assoc()['room_number'];
            }
            $get_current->close();
            
            // Update User
            $stmt = $conn->prepare("UPDATE users SET full_name = ?, email = ?, home_address = ?, phone_number = ?, room_number = ? WHERE id = ? AND role = 'student'");
            $stmt->bind_param('sssiii', $full_name, $email, $home_address, $phone_number, $room_number, $student_id);
            $stmt->execute();
            $stmt->close();
            
            // Update Rooms Table
            // Free Old Room
            if($current_room && $current_room != $room_number) {
                // Check Any Other student in Room
                $check_other = $conn->prepare("SELECT COUNT(*) as count FROM users WHERE room_number = ? AND id != ? AND role = 'student'");
                $check_other->bind_param('ii', $current_room, $student_id);
                $check_other->execute();
                $other_result = $check_other->get_result();
                $other_count = $other_result->fetch_assoc()['count'];
                $check_other->close();
                
                // If No Other Student, Mark as Available
                if($other_count == 0) {
                    $update_old = $conn->prepare("UPDATE rooms SET status = 'Available' WHERE room_number = ?");
                    $update_old->bind_param('i', $current_room);
                    $update_old->execute();
                    $update_old->close();
                }
            }
            
            // If New Room Assigned, Mark as Occupied
            if($room_number) {
                $update_new = $conn->prepare("UPDATE rooms SET status = 'Occupied' WHERE room_number = ?");
                $update_new->bind_param('i', $room_number);
                $update_new->execute();
                $update_new->close();
            }
...
<head>
  ...
  <title>Manage Students - Warden Portal</title>
